class ClientsController < ApplicationController
  ...
  def client_application_client_index
    ...
    @clientapplicationclients = Applicationclient.page(params[:page]).per(5)
   ...
   def client_application_client_create
    ...
    if @applicationclient.save
      @clientapplicationclients = Applicationclient.all
      @applications = Application.all
      @client = Client.find(params[:id]) 
  ...
  private
    ...
    def client_params
      params.require(:client).permit(:nit, :name, :reason, :addres, :telephone, :contact, :state, :page)
    end

    def applicationclient_params
      params.require(:applicationclient).permit(:user_id, :client_id, :state, :servicelevel, :application_id)
  end
end
