class IncidentsController < ApplicationController
  ...
  def edit
    ...
    @incidentmanagements = Incidentmanagement.joins(:incidents).where("incidents.user_id = #{current_login.id}").order("incidents.id DESC").page(params[:page]).per(5)
    @incident.picture
  end
  ...
  private
    ...
    def incident_params
      params.require(:incident).permit(:user_id, :area_id, :userapplication_id, :criticality_id, :datereport, :description, :state, :application_id, :applicationclient_id, :picture, :picture_url, :picture_identifier, :page)
    end
